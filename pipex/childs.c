...
/*   Updated: 2022/05/23 18:37:34 by dadoming         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child_processes(t_pipex p, char **argv, char **envp)
{
	...
	if (p.pid == 0)
	{
		if (p.child_num == 0)
		{
			pipe_dupper(p.infile, p.pipe_fd[STDOUT_FILENO]);
			if (p.here_doc == 1)
				unlink(".heredoc_storer");
		}
		else if (p.child_num == (p.command_counter) - 1)
			pipe_dupper(p.pipe_fd[2 * p.child_num - 2], p.outfile);
		else
			pipe_dupper(p.pipe_fd[2 * p.child_num - 2],
