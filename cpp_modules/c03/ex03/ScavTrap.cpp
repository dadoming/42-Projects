...
Scavtrap::Scavtrap( std::string name ) : Claptrap(name, 100, 20, 50, "ScavTrap") { std::cout << "Scavtrap constructor called" << std::endl; }

Scavtrap::Scavtrap( const Scavtrap& other ) : Claptrap(other) { std::cout << "Scavtrap copy constructor called" << std::endl; *this = other; }
...
void Scavtrap::guardGate()
{
    if (actionPossible())
    {
        std::cout << "\U0001f6e1\uFE0F" << "  " << getTrapType() << " " << getName() << " has entered Gate keeper mode" << std::endl;
        useEnergyPoint();
    }
    else
        unableToPerform(isDead(), getName(), getTrapType());
